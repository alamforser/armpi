...
# 6      5     4   3    2   1 
# [500, 600, 800, 110, 500,210]
pulse = transform.pulse2angle([818, 779, 1000, 190, 500])  # 舵机脉宽值转为弧度
#((2, int(pulse[4])), (3, int(pulse[3])), (4, int(pulse[2])), (5, int(pulse[1])), (6, int(pulse[0]))))
print('input:', pulse)
res = fk.get_fk(pulse)  #获取运动学正解
print('output:', res)
print('rpy:', transform.qua2rpy(res[1]))
# 设置连杆长度(m)base_link, link1, link2, link3, tool_link
fk.set_link(0.065, 0.1, 0.095, 0.05, 0.1)
...
res = get_ik([0.13095696510126037, 0.0, 0.0917891053187185], 0, [-180, 180])
if res != []:
    ...
    print('no solution')
# 设置连杆长度(m)base_link, link1, link2, link3, tool_link
set_link(0.065, 0.1, 0.095, 0.05, 0.1)
